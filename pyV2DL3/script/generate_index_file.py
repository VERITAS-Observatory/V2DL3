...
@click.option(
    '--folder_location', '-f', nargs=1, type=click.Path(exists=True),
    help='Path containing DL3 files.')
@click.option(
    '--index_file_dir', '-i', nargs=1, type=click.Path(exists=True),
    help='Output path to write the index file.')
@click.option(
    '--hdu_index_file', '-hdu', nargs=1,
    default='hdu-index.fits.gz',
    help='HDU index file (default:hdu-index.fits.gz)')
@click.option(
    '--obs_index_file', '-obs', nargs=1,
...
def cli(folder_location, index_file_dir,
    ...
    fits_files = [
        f for f in __fits_files
        if f.find(obs_index_file) == -1
        and f.find(hdu_index_file) == -1
    ]
    if not fits_files:
    ...
    logging.info(f'Generating index files {index_file_dir}/obs-index.fits.gz and {index_file_dir}/hdu-index.fits.gz')
