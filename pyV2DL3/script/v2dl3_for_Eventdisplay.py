...
@click.option(
    '--instrument_epoch', '-i', nargs=1, default=None,
    help='Instrument epoch to be stored in EVENTS header'
)
@click.option(
    '--save_multiplicity', '-m', is_flag=True,
    help='Save telescope multiplicity into event list'
)
@click.option(
    '--filename_to_obsid', '-I', is_flag=True,
    help='Override OBS_ID with output filename'
)
@click.option(
    '--evt_filter', type=click.Path(exists=True),
...
def cli(file_pair, full_enclosure, point_like,
    ...
    if filename_to_obsid:
        logging.info(f"Overwriting OBS_ID={hdulist[1].header['OBS_ID']} with OBS_ID={fname_base}")
