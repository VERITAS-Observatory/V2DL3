...
def fill_bintablehdu(
    ...
    hdu.name = hdu_name
    hdu = addHDUClassKeyword(hdu, class1='RESPONSE',
                             class2=class_2,
                             class3=class_3,
                             class4=class_4)
    ...
    if hdu_name == 'EFFECTIVE AREA':
        ...
        hdu.header.set('LO_THRES', response_dict['LO_THRES'],
                       'Low energy threshold of validity [TeV]')
        hdu.header.set('HI_THRES', response_dict['HI_THRES'],
...
def fillRESPONSE(datasource, instrument_epoch=None):
    ...
    if datasource.__irf_to_store__['point-like']:
        ...
        response_hdus.append(fill_bintablehdu(
                  'FULL_EA', 'EFFECTIVE AREA', 'EFF_AREA',
                  'FULL-ENCLOSURE', 'AEFF_2D',
                  response_dict, evt_dict, epoch_str)
                         )
        response_hdus.append(fill_bintablehdu(
                  'FULL_MIGRATION', 'ENERGY DISPERSION', 'EDISP',
                  'FULL-ENCLOSURE', 'EDISP_2D',
                  response_dict, evt_dict, epoch_str)
                         )
        response_hdus.append(fill_bintablehdu(
                  'PSF', 'PSF', 'PSF',
                  'FULL-ENCLOSURE', 'PSF_TABLE',
                  response_dict, evt_dict, epoch_str)
                         )
