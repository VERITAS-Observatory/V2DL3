...
def get_irf_not_safe(manager, offset_arr, az, ze, noise):
    ...
    ea_data_dict['ThetaLow'] = offset_np_arr
    ea_data_dict['ThetaHigh'] = offset_np_arr
    ea_data_dict['Data'] = ea
    ebias = np.array(ebias)
    ebias_data_dict['ELow'] = ebias_energy_low
    ebias_data_dict['EHigh'] = ebias_energy_high
    ebias_data_dict['ThetaLow'] = offset_np_arr
    ebias_data_dict['ThetaHigh'] = offset_np_arr
    ...
    abias_data_dict['ThetaLow'] = offset_np_arr
    abias_data_dict['ThetaHigh'] = offset_np_arr
...
class IRFLoader:
    def __init__(self,vts_io):
        ...
        self.__axis__ = ['Azimuth', 'Zenith', 'Noise', 'Offset']
        self.__buildIndex__()

    def __buildIndex__(self):
        manager = self.__manager__
        if (len(manager.fEffectiveArea) <= 0):
            raise Exception("No effective areas! ")
        index_check = manager.fEffectiveArea.at(0).fDimensionNames
        for k in self.__axis__:
            if (k not in index_check):
                raise Exception("IRF missing axis: {}".format(k))
        index_dict = {'Index': []}
        for i, ea in enumerate(manager.fEffectiveArea):
    ...
    def getIRF(self,az, ze, noise):
        ...
        irf_data = []
        offset_index = self.__axis_dict__['Offset']
        ...
        ea_data_peek = irf_data[0]['EA_Dict']['Data']
        ebias_data_peek = irf_data[0]['EBias_Dict']['Data']
        abias_data_peek = irf_data[0]['ABias_Dict']['Data']

        ea_array = np.zeros([2, 2, 2, *(ea_data_peek.shape)])
        ebias_array = np.zeros([2, 2, 2, *(ebias_data_peek.shape)])
        abias_array = np.zeros([2, 2, 2, *(abias_data_peek.shape)])
        ...
        for irf in irf_data:
            ...
            ea_array[index[0], index[1], index[2]] = ea_data
            ebias_array[index[0], index[1], index[2]] = ebias_data
            abias_array[index[0], index[1], index[2]] = abias_data
        inter_axis = np.array([[az_low, az_high],
                               [ze_low, ze_high],
                               [noise_low, noise_high]])
        ea_interpolator = RegularGridInterpolator(inter_axis, ea_array)
        ebias_interpolator = RegularGridInterpolator(inter_axis, ebias_data)
        abias_interpolator = RegularGridInterpolator(inter_axis, abias_data)
