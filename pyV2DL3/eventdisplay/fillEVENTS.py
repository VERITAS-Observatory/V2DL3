...
def __fillEVENTS__(edFileIO, select={}):
    ...
    with uproot.open(edFileIO) as file:
        ...
        telConfig = file['run_{}/stereo/telconfig'
                         .format(runNumber)].arrays(library='np')

        # Get start and stop time within the run.
        start_mjd = file['total_1/stereo/tRunSummary/MJDrunstart'] \
            .array(library='np')[0]
        stop_mjd = file['total_1/stereo/tRunSummary/MJDrunstop'] \
            .array(library='np')[0]

        # convert mjd to fits format
        t_start_fits = Time(start_mjd, format='mjd', scale='utc') \
            .to_value('fits')
        t_stop_fits = Time(stop_mjd, format='mjd', scale='utc') \
            .to_value('fits')
        deadtime = file['total_1/stereo/tRunSummary/DeadTimeFracOn'] \
            .array(library='np')[0]
        ...
        seconds_from_reference = \
            (Time(np.trunc(start_mjd), format='mjd', scale='utc') - t_ref).sec

        tstart_from_reference = \
            (Time(start_mjd, format='mjd', scale='utc') - t_ref).sec
        tstop_from_reference = \
            (Time(stop_mjd, format='mjd', scale='utc') - t_ref).sec

        DL3EventTree = file['run_{}/stereo/DL3EventTree'.format(runNumber)] \
            .arrays(library='np')
        ...
        if time_of_day.max() > 24*60*60:
            raise ValueError('Max value in time_of_day  \
        ...
        if select:
            ...
            for key, value in select.items():
                if isinstance(value, (list, tuple)):
                    ...
                    raise TypeError("select condition required \
        ...
        avAz = np.rad2deg(np.arctan2(np.sum(np.sin(avAz_rad)),
                          np.sum(np.cos(avAz_rad))))
        avAz = avAz if avAz > 0 else avAz + 360

        # Calculate circular mean RA and average DEC
        pointingDataReduced = \
            file["run_{}/stereo/pointingDataReduced".format(runNumber)] \
            .arrays(library='np')
        avRA = \
    ...
    return ({'goodTimeStart': [tstart_from_reference],
